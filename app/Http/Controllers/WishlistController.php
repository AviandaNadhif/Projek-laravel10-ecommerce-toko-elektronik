...
class WishlistController extends Controller
{
    ...
    public function wishlist(Request $request){
        ...
        if($already_wishlist) {
            request()->session()->flash('error','Anda sudah menambahkan ke wishlist');
            return back();
        }else{
            ...
            if ($wishlist->product->stock < $wishlist->quantity || $wishlist->product->stock <= 0) return back()->with('error','Stok tidak mencukupi!');
            $wishlist->save();
        }
