...
{{-- pie chart: tampilkan jumlah pesanan per hari dalam 7 hari terakhir (label bahasa Indonesia) --}}
<script type="text/javascript">
  // Load dan gambar pie chart berdasarkan data dari endpoint /order-day
  google.charts.load('current', {'packages':['corechart']});
  google.charts.setOnLoadCallback(drawChart);

  function drawChart()
  {
    fetch("{{ route('product.order.day') }}")
    .then(function(response){ return response.json(); })
    .then(function(analytics){
      // analytics is expected as: [['Hari','Pesanan'], ['Senin',5], ...]
      var data = google.visualization.arrayToDataTable(analytics);
      var options = {
        title : 'Pesanan (7 hari terakhir)',
        legend: { position: 'right' },
        height: 320,
        // warna slice pie (ubah sesuai preferensi hex)
        colors: ['#4e73df', '#1cc88a', '#36b9cc', '#f6c23e', '#e74a3b', '#858796', '#2ecc71']
      };
