                        ...
                        <ul class="list-main">
                            ...
                            @auth 
                                @if(Auth::user()->role=='admin')
                                    <li><i class="ti-user"></i> <a href="{{route('admin')}}"  target="_blank">Dasbor</a></li>
                                @else 
                                    <li><i class="ti-user"></i> <a href="{{route('user')}}"  target="_blank">Dasbor</a></li>
                                @endif
                ...
                <div class="col-lg-2 col-md-2 col-12">
                    ...
                    <div class="logo">
                        @php
                            $settings=DB::table('settings')->get();
                        @endphp                    
<img src="{{asset('backend/img/logo.png')}}" alt="#" width="50" height="50"> </a>
                    </div>
                    <!--/ End Logo -->
                    <!-- Search Form -->
                                    ...
                                    <ul class="shopping-list">
                                        {{-- {{Helper::getAllProductFromCart()}} --}}
                                            @foreach(Helper::getAllProductFromWishlist() as $data)
                                                    @php
                                                        $wphotos = array_filter(array_map('trim', explode(',', $data->product['photo'] ?? '')));
                                                        if (empty($wphotos)) { $wphotos = ['images/placeholder.png']; }
                                                        $wimg = preg_match('/^https?:\/\//i', $wphotos[0]) ? $wphotos[0] : asset($wphotos[0]);
                                                    @endphp
                                                    <li>
                                                        <a href="{{route('wishlist-delete',$data->id)}}" class="remove" title="Remove this item"><i class="fa fa-remove"></i></a>
                                                        <a class="cart-img" href="#"><img src="{{ $wimg }}" alt="{{ $data->product['title'] }}"></a>
                            ...
                            @auth
                                <div class="shopping-item">
                                    ...
                                    <ul class="shopping-list">
                                        {{-- {{Helper::getAllProductFromCart()}} --}}
                                            @foreach(Helper::getAllProductFromCart() as $data)
                                                    @php
                                                        $cphotos = array_filter(array_map('trim', explode(',', $data->product['photo'] ?? '')));
                                                        if (empty($cphotos)) { $cphotos = ['images/placeholder.png']; }
                                                        $cimg = preg_match('/^https?:\/\//i', $cphotos[0]) ? $cphotos[0] : asset($cphotos[0]);
                                                    @endphp
                                                    <li>
                                                        <a href="{{route('cart-delete',$data->id)}}" class="remove" title="Remove this item"><i class="fa fa-remove"></i></a>
                                                        <a class="cart-img" href="#"><img src="{{ $cimg }}" alt="{{ $data->product['title'] }}"></a>
                                                        <h4><a href="{{route('product-detail',$data->product['slug'])}}" target="_blank">{{$data->product['title']}}</a></h4>
                                                        <p class="quantity">{{$data->quantity}} x - <span class="amount">Rp {{number_format($data->price,0,',','.')}}</span></p>
                                                    </li>
                                            @endforeach
                                    ...
                                    <div class="bottom">
                                        ...
                                        <a href="{{route('checkout')}}" class="btn animate">Bayar</a>
                                    </div>
                                </div>
                            @endauth
                            <!--/ End Shopping Item -->
