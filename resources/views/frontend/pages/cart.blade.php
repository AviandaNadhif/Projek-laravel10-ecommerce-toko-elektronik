							...
							<form action="{{route('cart.update')}}" method="POST">
								...
								@if(Helper::getAllProductFromCart())
									@foreach(Helper::getAllProductFromCart() as $key=>$cart)
										<tr>
											@php
											$cphotos = array_filter(array_map('trim', explode(',', $cart->product['photo'] ?? '')));
											if (empty($cphotos)) { $cphotos = ['images/placeholder.svg']; }
											$cclean = ltrim($cphotos[0], '/'); $cimg = preg_match('/^https?:\/\//i', $cphotos[0]) ? $cphotos[0] : asset($cclean);
											@endphp
											<td class="image" data-title="No"><img src="{{ $cimg }}" alt="{{ $cart->product['title'] }}"></td>
								...
								@else
										<tr>
											<td class="text-center">
												Tidak ada item dalam keranjang. <a href="{{route('product-grids')}}" style="color:blue;">Lanjut belanja</a>
							...
							<div class="col-lg-4 col-md-7 col-12">
								<div class="right">
									<ul>
										...
										<li class="coupon_price" data-price="{{Session::get('coupon')['value']}}">Anda Hemat<span>Rp {{number_format(Session::get('coupon')['value'],0,',','.')}}</span></li>
										@endif
										@php
										...
										@endphp
										@if(session()->has('coupon'))
											<li class="last" id="order_total_price">Total Bayar<span>Rp {{number_format($total_amount,0,',','.')}}</span></li>
										@else
											<li class="last" id="order_total_price">Total Bayar<span>Rp {{number_format($total_amount,0,',','.')}}</span></li>
										@endif
									</ul>
									<div class="button5">
										<a href="{{route('checkout')}}" class="btn">Bayar</a>
	...
	<script>
		$(document).ready(function(){
			$('.shipping select[name=shipping]').change(function(){
				...
				// alert(coupon);
				$('#order_total_price span').text('Rp '+(subtotal + cost-coupon).toLocaleString('id-ID'));
			});
